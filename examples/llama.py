...
if Device.DEFAULT == "CPU" and not getenv("CPU"):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run LLaMA 7B in tinygrad", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    parser.add_argument(
        "--prompt", type=str, default=None, help="Phrase to start with. Without this, it goes into chatbot mode"
    )
    ...
    if args.large:
        ...
        with Timing(
            ...
            weights0 = fake_torch_load_zipped(
                open(WEIGHTS0_FILENAME, "rb"), load_weights=getenv("WEIGHTS", 1), base_name="consolidated.00"
            )
            weights1 = fake_torch_load_zipped(
                open(WEIGHTS1_FILENAME, "rb"), load_weights=getenv("WEIGHTS", 1), base_name="consolidated.01"
            )
        ...
        for k, v in (t := tqdm(weights0.items())):
            ...
            if w0.shape[0] != mv.shape[0]:
                mv.assign(w0t.cat(w1t, dim=0))
            elif w0.shape[1] != mv.shape[1]:
                mv.assign(w0t.cat(w1t, dim=1))
            else:
        ...
        with Timing(
            ...
            weights = fake_torch_load_zipped(
                open(WEIGHTS_FILENAME, "rb"), load_weights=getenv("WEIGHTS", 1), base_name="consolidated"
            )
